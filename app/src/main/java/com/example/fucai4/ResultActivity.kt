...
import kotlinx.android.synthetic.main.act_resul.*
...
class ResultActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.act_resul)
        ...
        when (tpye) {
            "0" -> { //福彩计算方法
                ...
                mian_et_can1 = intent.getStringExtra("mian_et_can1")
                mian_et_can2 = intent.getStringExtra("mian_et_can2")
                mian_et_can3 = intent.getStringExtra("mian_et_can3")
            ...
            "1" -> {//体彩计算方法
                ...
                resul_tv_cankao.visibility = View.INVISIBLE
                resul_tv_shuo.setText(shuoming)
                resul_et_shang.setText(shang)
            }
        }

        resul_gv_z6.adapter = GridViewSim(t6, this@ResultActivity, 1)
        resul_gv_z3.adapter = GridViewSim(t3, this@ResultActivity, 1)
        a = t6
        b = t3
        tv_z6.setOnClickListener { v: View? ->
            var op = a.toString().replace("[", "")
            op = op.replace("]", "")
            op = op.replace(",", "")

            mClipboardManager!!.primaryClip = ClipData.newPlainText(null, op.toString() + "（来自组选王  微信:aawwss1788)")
            var tos = Toast.makeText(this@ResultActivity, "复制成功了组6的全部号码", Toast.LENGTH_SHORT)
            tos.setGravity(Gravity.CENTER, 0, 0)
            tos.show()
        }

        tv_z3.setOnClickListener { v: View? ->
            var op = b.toString().replace("[", "")
            op = op.replace("]", "")
            op = op.replace(",", "")

            mClipboardManager!!.primaryClip = ClipData.newPlainText(null, op.toString() + "（来自组选王  微信:aawwss1788)")
            var tos = Toast.makeText(this@ResultActivity, "复制成功了组3的全部号码", Toast.LENGTH_SHORT)
            tos.setGravity(Gravity.CENTER, 0, 0)
            tos.show()
        }
        ...
        resul_suan.setOnClickListener { v: View? ->
            ...
            if (shang2.length != 3) {
                var tos = Toast.makeText(this@ResultActivity, "参考号码必须是三位数，不然无法显示", Toast.LENGTH_SHORT)
                tos.setGravity(Gravity.CENTER, 0, 0)
                tos.show()
...
//                        Log.e("新组6数据", a6.toString())
                            ...
                            if (i == 4) { //胆组执行
                                if (value != "") {
                                    ...
                                    if (danzu_text.size > 1 && danzu_text.size <= 3) {
                                        ...
                                        Toast.makeText(this@ResultActivity, "最少输入2组胆组,最多输入3组胆组", Toast.LENGTH_SHORT).show()
                                        return@setOnClickListener
                                    }
                        ...
                        resul_gv_z6.setAdapter(GridViewSim(a as java.util.ArrayList<String>?, this@ResultActivity, 2))
                        resul_gv_z3.setAdapter(GridViewSim(b as java.util.ArrayList<String>?, this@ResultActivity, 2))
...
//                                fuzhi_msg?.text = lin_b3[2].toString()
        ...
        resul_yuan.setOnClickListener { v: View? ->
            var intent = Intent(this@ResultActivity, OriginalActivity::class.java)
